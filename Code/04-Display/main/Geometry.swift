...
struct Point {
	...
	func movedBy(offsetX: some BinaryInteger, offsetY: some BinaryInteger) -> Point {
		Point(x: x.value + Int(offsetX), y: y.value + Int(offsetY))
	}
}
...
extension Rect {
	...
	var width: Int { size.width.value }
	var height: Int { size.height.value }
